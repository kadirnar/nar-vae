...
| Reference-conditioning cache | Yes | Avoid re-encoding an unchanged reference within an authorized session | Key by exact reference bytes, preprocessing version, checkpoint, language metadata, and precision. Keep it bounded and non-persistent by default because voice references are sensitive. |
