...
def bind_training_dataset_identity(
    ...
    from vyvotts.dataset.identity import validate_training_dataset_identity
...
def _validate_run_manifest(
    ...
    *,
    ...
    from vyvotts.dataset.identity import training_dataset_identity_sha256
...
def resolve_training_reporters(value: Any) -> tuple[str, ...]:
    """Return supported Trainer reporters; ``none`` means no external service."""
    ...
    if isinstance(value, str):
        ...
        raise ValueError("report_to must be 'none', 'wandb', or a list containing 'wandb'.")

    unsupported = sorted(set(requested) - set(TRAINING_REPORTERS))
    if unsupported:
        raise ValueError(
            f"Unsupported training reporter(s): {unsupported}. "
            "This build supports only 'wandb' or 'none'."
        )
...
def build_training_argument_overrides(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    reporters = resolve_training_reporters(config.get("report_to", "none"))
...
def load_inference_settings() -> InferenceSettings:
    ...
    resource = resources.files("vyvotts.configs").joinpath("inference.toml")
