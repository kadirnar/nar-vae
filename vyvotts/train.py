...
from vyvotts.model_manifest import representation_from_config, write_model_manifest
...
try:
    ...
    # Keep the module importable in the lightweight inference install. Calling
    # the training API still fails early with a useful dependency hint.
...
def _pretrain(
    ...
    if _TRAINING_IMPORT_ERROR is not None:
        raise RuntimeError(
            "Training dependencies are unavailable. Install the bounded training "
    ...
    # Trainer's W&B callback owns logging and runs only on the world process zero.
    run_distributed_operation(
        process,
        lambda: _configure_pretraining_wandb_environment(
            config,
            enabled=process.is_main_process and "wandb" in training_overrides["report_to"],
        ),
