...
def _require_datasets() -> None:
    """Raise an actionable error when optional dataset tools are unavailable."""
    if _DATASETS_IMPORT_ERROR is not None:
        raise RuntimeError(
            "Dataset preparation dependencies are unavailable. Install them with "
            "`pip install 'nar-vae[train]'`."
