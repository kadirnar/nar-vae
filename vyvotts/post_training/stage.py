...
from vyvotts.configuration import load_training_lineage
...
from vyvotts.model_manifest import ModelManifest, validate_manifest_weight, write_model_manifest
...
def load_grpo_stage_config(path: str | os.PathLike[str]) -> GRPOStageConfig:
    ...
    try:
        ...
        raise RuntimeError("GRPO training requires PyYAML from `nar-vae[train]`.") from exc
...
def run_grpo_stage(
    ...
    *,
    ...
    try:
        logger = _WandbLogger(
            config,
            enabled=context.is_main_process and config.report_to == "wandb",
        )
    except Exception as exc:
        logger_error = exc
