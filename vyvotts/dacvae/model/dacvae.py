...
from ..nn.bottleneck import VAEBottleneck
from ..nn.layers import MsgProcessor, NormConv1d, NormConvTranspose1d, Snake1d, activation
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        chunks = [
            self.block[i : i + self._chunk_size] for i in range(0, layer_cnt, self._chunk_size)
        ]
        group = [
            layer for j, chunk in enumerate(chunks) if j % self._chunk_size == 0 for layer in chunk
        ]
...
class Decoder(nn.Module):
    ...
    def forward(self, x, message: torch.Tensor | None = None):
        for layer in self.model:
            x = layer(x)
        return self.watermark(x, message)

    def watermark(self, x, message: torch.Tensor | None = None):
        if self.alpha == 0.0:
            return x
...
class DAC(BaseModel, CodecMixin):
    ...
    def decode(self, z: torch.Tensor, message: torch.Tensor | None = None):
