...
from nar_vae.checkpoint import load_pretrained_checkpoint
...
def _require_wandb() -> None:
    """Import the mandatory reporter lazily when SFT starts."""
    try:
        import_module("wandb")
    except (ImportError, RuntimeError) as exc:
        raise RuntimeError(
            "W&B is required for SFT but wandb is unavailable. "
...
def _finetune(
    ...
    if _TRAINING_IMPORT_ERROR is not None:
        raise RuntimeError(
            "Fine-tuning dependencies are unavailable. Reinstall the single package with "
