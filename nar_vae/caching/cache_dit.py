...
from nar_vae.configuration import CACHE_DIT_MIN_STEPS
...
def _load_cache_dit() -> ModuleType:
    try:
        ...
        raise CacheDiTUnavailableError(
            "The turbo profile requires Cache-DiT. Reinstall the single package with "
    ...
    if missing:
        ...
        raise CacheDiTUnavailableError(
            ...
            "Reinstall the single package with `python -m pip install -e .`."
        )
    return module
